...
def main(args):
    ...
    try:
        scan = ET.parse(args.nessusFile)
    except:
        print("Unable to open file ", args.nessusFile)
        exit()
    ...
    for reportHost in bar:
        ...
        for report in repItems:
            report["filename"] = args.nessusFile.split("/")[-1] # Add Filename to Report
            if args.CATEGORY:
    ...
    # Save to File
    with open(f"{args.nessusFile}.csv", "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('nessusFile', type=str, help='nessus file')
    # parser.add_argument('-w', '--writeFile', help='path to write file')
